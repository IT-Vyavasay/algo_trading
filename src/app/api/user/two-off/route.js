...
export async function POST(req, res) {
  try {
    ...
    let user_data = await sql_query(
      `select u.userId,ud.isTwoFa from tbluser as u ,tbluserdetailofsolares as ud where u.userId=ud.userId and u.userId = ? `,
      [user_Id],
    );
    ...
    if (user_data) {
      if (user_data.isTwoFa == 0) {
        return NextResponse.json(
          { message: 'Two factor already disable for this user' },
          { status: 400 },
        );
      } else {
        let now = get_timestemp();
        await sql_query(
          `update tbluserdetailofsolares set isTwoFa=?, twoFaCode=?, updatedOn=? where  userId =? `,
          [0, null, now, user_data?.userId],
        );
        return NextResponse.json(
