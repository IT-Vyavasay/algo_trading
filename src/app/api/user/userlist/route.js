...
export async function GET(req, res) {
  ...
  try {
    ...
    query += `SELECT userId , username, email ,status, isVerify , lastLoginIp  , createdOn   from tbluser`;
    ...
    if (users.length > 0) {
      let get_userName = await sql_query(
        `SELECT isTwoFa, userId FROM tbluserdetailofsolares`,
        [],
        'Multi',
      );
      allData = users.map((j, k) => {
        let userIndex =
          get_userName && get_userName.findIndex(x => x.userId == j.userId);
        ...
        return {
          ...
          lastLoginDate: j.createdOn,
          siteType: j.createdOn,
        };
      });
    }
